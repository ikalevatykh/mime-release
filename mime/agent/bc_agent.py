...
@click.option('-np', '--net_path', type=str, required=True,
              help='net path')
@click.option('-ne', '--net_epoch', type=str, default='current',
              help='net epoch to load')
@click.option('-v', '--video', type=str, default='',
              help='record a video of the demonstration')
@click.option('-va', '--video-agent', type=str, default='',
              help='record a video of the demonstration')
@click.option('-pa', '--pickle-actions/--no-pickle-actions', default=False,
              help='whether to pickle the actions')
@click.option('-pn', '--pickle-name', type=str, default=None,
              help='name of the pickled actions')
@click.option('-d', '--device', type=str, default='cuda', help='cuda or cpu')
@click.option('-tsc', '--timescales', type=json.loads,
              default="{\"0\": 50, \"1\": 50, \"2\":200, \"3\":200}",
              help='timescale(s) of the skill choice')
@click.option('-cv2/-ncv2', '--use-cv2/--no-use-cv2', default=False,
              help='whether to show images with opencv')
@click.option('-r/-nr', '--render/--no-render', default=True,
              help='whether to render the agent')
@click.option('-ss', '--skill-sequence', type=json.loads, default="[0, 2, 3, 1, 2, 3]",
...
@click.option('-ia', '--image-augmentation', type=str, default='',
              help='image augmentation to use with the agent')
def main(env_name, seed, net_path, net_epoch, device, video, pickle_actions, video_agent,
         pickle_name, timescales, use_cv2, render, skill_sequence, image_augmentation):
    if use_cv2:
        import cv2
    ...
    agent = BCAgent(
        net_path, net_epoch, max_steps=-1, device=device,
        augmentation_str=image_augmentation)
    is_skill_policy = 'skills' in agent.model.args['mode'] or 'film' in agent.model.args['mode']
    ...
    while action is not None:
        ...
        if is_skill_policy and (step_after_skill == 0 or
                                (skill is not None and step_after_skill == timescales[skill])):
    ...
    if pickle_actions:
        ...
        if pickle_name is None:
            ...
            rollout_path = os.path.join(rollouts_dir, '{}_{}.pkl'.format(
                pickle_name, seed))
