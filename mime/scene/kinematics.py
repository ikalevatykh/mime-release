...
class DefaultKinematics:
    ...
    def inverse(self, pos, orn, q_init=None):
        ...
        joint_pos = pb.calculateInverseKinematics(
            self._chain.body_id,
            self._chain.tip.link_index,
            pos,
            orn,
            jointDamping=self._damping,
            lowerLimits=self._chain._lowers,
            upperLimits=self._chain._uppers,
            jointRanges=self._chain._ranges,
    ...
    def inverse_all(self, pos, orn, p_tol=1e-5, o_tol=1e-3, max_iter=100):
        ...
        for i in range(max_iter):
            ...
            if np.allclose(p, pos, atol=p_tol) and _quatclose(o, orn, atol=o_tol):
                q_sol.append(q)
