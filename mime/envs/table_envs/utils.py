...
class MeshLoader:
    def __init__(self,
        ...
        self.files = []
        self.dic_labels = json.load(
            open(os.path.join(self.root, 'labels.json'), 'r'))

        for folder, ratio in zip(folders, ratios):
            self.files_folder = [
                os.path.join(folder, file)
                for file in self.dic_labels[folder]['train']+self.dic_labels[folder]['test']
            ]
            self.files_folder = self.files_folder[:int(ratio *
                                                       len(self.files_folder))]
    ...
    def get_mesh(self, idx, scale, reduced_radius_model=False, useFixedBase=False):
        if self.simplified and os.path.exists(
            ...
            if self.reduced_radius_model and os.path.exists(
                os.path.join(self.root, self.files[idx], 'models', 'model_simplified_radius.urdf')):
...
def sample_without_overlap(mesh,
                           aabbs,
                           np_random,
                           low,
                           high,
